...
cd /d "%~dp0"
title FIFA 15 Remote Player - f15b - Joiner Result V13

set "EXPECTED_BRANCH=integration/test-matchmaking-b-joiner-result-v13"
set "EXPECTED_HOST_BRANCH=integration/test-matchmaking-joiner-result-v13"
...
rem them before FIFA exists. v13 never starts or self-tests the native observer.
...
echo.
echo ============================================================
echo   FIFA15 PLAYER B - JOINER RESULT V13
echo ============================================================
echo   Player B: %EXPECTED_BRANCH%
echo   Paired host: %EXPECTED_HOST_BRANCH%
echo   Host wire delta: A REAS.RSLT = 0 SUCCESS_CREATED_GAME.
echo   Joiner wire delta: B REAS.RSLT = 1 SUCCESS_JOINED_NEW_GAME.
echo   Retained: v12 bilateral promotion-time JoinCompleted.
...
echo.
echo V13 crash guard active: native GSU/Frida observer is intentionally NOT started.
...
if not "%RC%"=="0" (
  ...
  echo.
  echo Test finished against joiner-result host v13 with passive network diagnostics only.
  echo Send the newest FIFA15-F15B-EVIDENCE-*.zip from the Desktop.
)
exit /b %RC%
