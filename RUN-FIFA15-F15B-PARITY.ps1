<#
Player B runner for the working-server peer session v5 test.
...
if ($SelfTest) {
    ...
    foreach ($token in $banned) {
        if ($source.Contains($token)) {
            throw "Player B peer-session v5 runner reintroduced in-process instrumentation: $token"
        }
    }
    if ($source -match "Start-Process\s+-FilePath\s+'python'") {
        throw 'Player B peer-session v5 runner reintroduced a Python observer process.'
    }
    Write-Host "PASS: Player B peer-session v5 runner pins $ExpectedBranch / $Candidate / $Package, attaches nothing to fifa15.exe, and retains the known-good boot/connect stack." -ForegroundColor Green
    exit 0
}

New-Item -ItemType Directory -Force -Path $attempt | Out-Null
@(
    'FIFA15 working-server peer session v5 - Player B',
