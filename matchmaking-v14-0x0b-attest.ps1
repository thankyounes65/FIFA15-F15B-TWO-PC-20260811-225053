...
$PackageRevision = 'ea-readiness-v1+loopback-forwarder-17502-17503+matchmaking-v14-0x0b-attest2'
...
function Get-Contract {
    ...
    if ([bool]$manifest.matchmaking_overlay.requires_git_checkout) { throw 'Portable v14 Player B package unexpectedly requires a Git checkout.' }
...
function Test-Contract {
    ...
    Write-Host 'PASS: portable Player B package carries the exact FIFA15-MM-V14-0X0B contract.' -ForegroundColor Green
...
function Connect-CandidateGate([Net.Sockets.TcpClient]$Client,[string]$HostIp,[int]$Port) {
    ...
    try {
        ...
        if (-not $Client.Connected) { throw "Player A v14 candidate gate $HostIp`:$Port did not establish a TCP connection." }
    }
    catch {
...
try {
    ...
    if ($ack -ne $expectedAck) { throw "Player A did not accept exact v14 Player B package token; got '$ack'." }
