...
function Assert-LocalState {
    ...
    foreach ($path in @(
        ...
        if (-not (Test-Path -LiteralPath (Join-Path $Root $path) -PathType Leaf)) {
            throw "Missing Player B observer prerequisite: $path"
        }
    }
...
Assert-LocalState
if($SelfTest){
    Write-Host "PASS: portable scenario-free Player B observer attestation pins package=$Package host=$HostIp`:$Port and validates the native-observer package contract without requiring Git." -ForegroundColor Green
    exit 0
}
